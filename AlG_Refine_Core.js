// AlGzawy - بوت الصقل المطور (الكود الأساسي النهائي)
(function() {
    'use strict';

    // --- 1. نظام الإعدادات (يعتمد على نفسه مرة أخرى) ---
    ...
    const SETTINGS = {
        get: (key) => GM_getValue(SETTINGS_PREFIX + key, DEFAULTS[key]),
        set: (key, value) => GM_setValue(SETTINGS_PREFIX + key, value)
    };

    // --- 2. متغيرات التشغيل ---
    let currentTimeoutId = null;
    let isBotActive = SETTINGS.get('isRunning');
    ...
    function createSettingsPanel() {
        const panelHTML = `
            <div id="algzawy-refine-panel" style="position: fixed; top: ${SETTINGS.get('panelTop')}; left: ${SETTINGS.get('panelLeft')}; z-index: 9998; background-color: #f4e4bc; border: 2px solid #7d510f; border-radius: 8px; font-family: 'Trebuchet MS', sans-serif; width: 280px; direction: rtl;">
        ...
        $('#step-market').prop('checked', SETTINGS.get('stepMarket'));
        $('#step-academy').prop('checked', SETTINGS.get('stepAcademy'));
        $('#schedule-market').prop('checked', SETTINGS.get('scheduleMarket'));
        $('#market-interval').val(SETTINGS.get('marketInterval'));
        $('#min-delay').val(SETTINGS.get('minDelay'));
        $('#max-delay').val(SETTINGS.get('maxDelay'));
        $('#min-retry').val(SETTINGS.get('minRetry'));
        $('#max-retry').val(SETTINGS.get('maxRetry'));

        $("#algzawy-refine-panel").draggable({ handle: "#algzawy-refine-header", stop: (e, ui) => {
            SETTINGS.set('panelTop', ui.position.top + 'px');
            SETTINGS.set('panelLeft', ui.position.left + 'px');
        }});
        ...
        $('#save-settings-btn').on('click', function() {
            SETTINGS.set('stepMarket', $('#step-market').is(':checked'));
            SETTINGS.set('stepAcademy', $('#step-academy').is(':checked'));
            SETTINGS.set('scheduleMarket', $('#schedule-market').is(':checked'));
            SETTINGS.set('marketInterval', parseInt($('#market-interval').val()));
            SETTINGS.set('minDelay', parseInt($('#min-delay').val()));
            SETTINGS.set('maxDelay', parseInt($('#max-delay').val()));
            SETTINGS.set('minRetry', parseInt($('#min-retry').val()));
            SETTINGS.set('maxRetry', parseInt($('#max-retry').val()));
            alert('تم حفظ الإعدادات.');
        });

        $('#toggle-run-btn').on('click', function() {
            isBotActive = !isBotActive;
            SETTINGS.set('isRunning', isBotActive);
            updateToggleButton();
    ...
    async function main() {
        ...
        const currentSettings = {
            stepMarket: SETTINGS.get('stepMarket'),
            stepAcademy: SETTINGS.get('stepAcademy'),
            scheduleMarket: SETTINGS.get('scheduleMarket'),
            marketInterval: SETTINGS.get('marketInterval'),
            minDelay: SETTINGS.get('minDelay'),
            maxDelay: SETTINGS.get('maxDelay'),
            minRetry: SETTINGS.get('minRetry'),
        ...
        if (currentSettings.scheduleMarket && currentSettings.stepMarket) {
            const lastRun = SETTINGS.get('lastMarketRun');
            ...
            if (timeToRunMarket) {
                if (location.href.includes('screen=market&mode=call')) {
                    ...
                    if (success) {
                        SETTINGS.set('lastMarketRun', Date.now());
    ...
    function init() {
        if (typeof $ !== 'undefined' && (location.href.includes('screen=market&mode=call') || location.href.includes('screen=snob'))) {
            $(document).one('click', () => {
