(function(SETTINGS, game_data, $) {
    ...
    function createSettingsPanel() {
        const panelHTML = `
            <div id="algzawy-refine-panel" style="position: fixed; top: ${SETTINGS.panelTop}; left: ${SETTINGS.panelLeft}; z-index: 9998; background-color: #f4e4bc; border: 2px solid #7d510f; border-radius: 8px; font-family: 'Trebuchet MS', sans-serif; width: 280px; direction: rtl;">
                ...
                <div id="algzawy-refine-body" style="padding: 10px;">
                    <div style="display: grid; grid-template-columns: 1fr auto; gap: 8px 12px; align-items: center;">
                        ...
                        <label style="grid-column: 1 / -1; display: flex; align-items: center;"><input type="checkbox" id="active-tab-only" style="margin-left: 8px;">العمل في التاب النشط فقط</label>
                    </div>
                    ...
                    <p style="text-align: center; margin: 10px 0 0 0; font-size: 10px; color: #7d510f;">&copy; جميع الحقوق محفوظة - AlGzawy</p>
                </div>
            </div>
        `;
        $('body').append(panelHTML);

        $('#step-market').prop('checked', SETTINGS.stepMarket);
        $('#step-academy').prop('checked', SETTINGS.stepAcademy);
        $('#schedule-market').prop('checked', SETTINGS.scheduleMarket);
        $('#market-interval').val(SETTINGS.marketInterval);
        $('#min-delay').val(SETTINGS.minDelay);
        $('#max-delay').val(SETTINGS.maxDelay);
        $('#min-retry').val(SETTINGS.minRetry);
        $('#max-retry').val(SETTINGS.maxRetry);
        $('#active-tab-only').prop('checked', SETTINGS.activeTabOnly);
        ...
        $('#save-settings-btn').on('click', function() {
            ...
            SETTINGS.save('activeTabOnly', $('#active-tab-only').is(':checked'));
        ...
        $('#toggle-run-btn').on('click', function() {
            ...
            updateToggleButton();
            if (isBotActive) { main(); }
            else { clearTimeout(currentTimeoutId); }
        });
    }
    ...
    async function market() {
        ...
        if (!selectAllButton) { return true; }
    ...
    async function main() {
        ...
        if (SETTINGS.activeTabOnly && document.hidden) {
            console.log("التاب غير نشط، البوت متوقف مؤقتاً. سيعود للعمل عند التركيز على التاب.");
            currentTimeoutId = setTimeout(main, 5000); // يفحص كل 5 ثواني
            return;
        }

        if (detectBotDetection()) { return; }
        ...
        if (currentSettings.scheduleMarket && currentSettings.stepMarket) {
            const lastRun = currentSettings.lastMarketRun || 0;
...
})(unsafeWindow.ALGZAWY_SETTINGS, unsafeWindow.game_data, unsafeWindow.jQuery);
